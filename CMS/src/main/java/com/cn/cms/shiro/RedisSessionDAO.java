...
/**
 * @author: Farben
 * @description: RedisSessionDAO -参考 shiro-redis 开源项目 Git地址 https://github.com/alexxiyang/shiro-redis
 * @create: 2019/8/29-13:46
 **/
public class RedisSessionDAO extends AbstractSessionDAO {
    ...
    private static final String DEFAULT_KICKOUT_SESSION_KEY_PREFIX = "shiro:cache:kickout:";
    private String keyPrefix = DEFAULT_SESSION_KEY_PREFIX;
    private String keyKickoutPrefix = DEFAULT_KICKOUT_SESSION_KEY_PREFIX;

    private static final long DEFAULT_SESSION_IN_MEMORY_TIMEOUT = 160*1000L;
    /**
    ...
    /**
     * 获取session的数量
     * 由于请求登录页面，也会生成session缓存，不适于统计在线人数
     * @return
     */
     ...
     * @return
     */
    public Long getKickoutSessionSize() {
        Long size = 0L;
        try {
            size = redisManager.scanSize(this.keyKickoutPrefix + "*");
        } catch (Exception e) {
            logger.error("get kickout sessions error.");
        }
        return size;
    }
    ...
    protected Session doReadSession(Serializable sessionId) {
        ...
        try {
            s = (ShiroSession) redisManager.get(getRedisSessionKey(sessionId));
            //setSessionToThreadLocal(sessionId, s);
            Date now = new Date();
            long duration = now.getTime() - s.getLastAccessTime().getTime();
            // 间隔时间大于session快过期时间，标记更新.sessionInMemoryTimeout要比session过期时间小；
            if (duration > sessionInMemoryTimeout) {
                ((ShiroSession) s).setChanged(true);
            }
        } catch (Exception e) {
